...
final Class BTCChinaAPI
{
    ...
    private $accessKey = 'YouAccessKey';
    private $secretKey = 'YouSecretKey';
    private $ch = '';
    ...
    function __construct($access = '', $secret = '')
    {
        ...
        if ($access) $this->accessKey = $access;
        if($secret) $this->secretKey = $secret;
        $this->ch = curl_init();
    }
    ...
    */
    ...
    // market  string  否   可选值BTCCNY、LTCCNY、LTCBTC、ALL 默认值为BTCCNY
    // limit   integer 否   限制返回的交易记录数，默认为 1000。
    // offset  integer 否   分页索引, 默认为 0.
    // since   integer 否   限制返回交易记录的起始时间.
    // withdetail  boolean 否   是否返回订单内每笔交易详情。可选值true,false. 默认值为false，不返回交易详情
    ...
    // transaction    string  否   按类型获取交易记录。默认为“all”（全部）。可用类型包括： 'all|fundbtc| withdrawbtc|withdrawbtcfee|refundbtc|fundltc | withdrawltc|withdrawltcfee|refundltc|fundmoney | withdrawmoney| withdrawmoneyfee|refundmoney|buybtc|sellbtc|buyltc|sellltc| tradefee| rebate '
    // limit   integer 否   限制返回的交易记录数，默认为 10。
    // offset  integer 否   分页索引, 默认为 0.
    // since   integer 否   取从某一点开始的交易记录, 这一点可以是某个交易号或者Unix时间戳, 默认值为0.
    // sincetype   string  否   指定since参数的类型，可以是“id”或者“time”，默认值为“time”.
